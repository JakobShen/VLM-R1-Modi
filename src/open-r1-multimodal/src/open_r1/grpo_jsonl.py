...
model_path = "/openbayes/input/input2/Qwen2.5-0.5B"
local_rank = int(os.environ.get("LOCAL_RANK", 0))
device = torch.device(f"cuda:{local_rank}")
tokenizer_eval = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True)
model_eval = AutoModelForCausalLM.from_pretrained(
    model_path, trust_remote_code=True, torch_dtype=torch.float16,
    attn_implementation="flash_attention_2",
    device_map={"": local_rank}  # 👈 每个进程加载模型到自己独占的 GPU
)
gen_config = GenerationConfig(
    do_sample=False,
    max_new_tokens=64,
    eos_token_id=tokenizer_eval.eos_token_id,
    pad_token_id=tokenizer_eval.eos_token_id  # 添加这个以避免再次 warning
)
...
def evaluate_answer_quality(student_answer, ground_truth):
    prompt = (
        "You are a strict evaluation expert. "
        "Evaluate the following student's reasoning based on the following 5 strict criteria:\n\n"
        "1. The reasoning is detailed.\n"
        "2. The reasoning is complete.\n"
        "3. The reasoning does not skip intermediate steps.\n"
        "4. The reasoning includes image description.\n"
        "5. The reasoning includes analysis of answer choices.\n\n"
        "If **all five** criteria are clearly satisfied, output ONLY '1'.\n"
        "If **any** of the five are not fully satisfied, output ONLY '0'.\n"
        "No explanation or additional text is allowed.\n\n"
        f"Student's response: {student_answer}\nOutput only '1' or '0':"
    )

    device = torch.device(f"cuda:{torch.cuda.current_device()}")
    inputs = tokenizer_eval(prompt, return_tensors="pt", truncation=True, max_length=512)
    inputs = {k: v.to(device) for k, v in inputs.items()}

    with torch.no_grad():
        out = model_eval.generate(**inputs, generation_config=gen_config)

    # 只取生成的新部分
    input_len = inputs["input_ids"].shape[1]
    new_tokens = out[0][input_len:]
    result = tokenizer_eval.decode(new_tokens, skip_special_tokens=True).strip()

    print(f"[evaluate_answer_quality] model output: {result}")
    try:
        match = re.search(r"\s*[\"'`＊*]*([01])[\"'`＊*]*\s*", result)
        if match:
            return float(match.group(1))
        else:
            print("\033[91m[evaluate_answer_quality] No match found in result →", repr(result), "\033[0m")
            return 0.0
    except Exception as e:
        print(f"\033[91m[evaluate_answer_quality] Exception: {e} → result = {repr(result)}\033[0m")
        return 0.0


def llm_reward(content, sol, **kwargs):
    reward_mcq = mcq_reward(content, sol)
    # Extract answer from content if it has think/answer tags
    sol_match = re.search(r'<answer>(.*?)</answer>', sol)
    ground_truth = sol_match.group(1).strip() if sol_match else sol.strip()

    # Extract answer from content if it has think/answer tags
    content_matches = re.findall(r'<think>(.*?)</think>', content, re.DOTALL)
    student_answer = content_matches[-1].strip() if content_matches else content.strip()

    #print(student_answer)
    reward_llm = evaluate_answer_quality(student_answer, ground_truth)
    #print(reward_llm)
    reward_llm = reward_llm if isinstance(reward_llm, float) else 0.0

    return reward_llm * reward_mcq
...
def main(script_args, training_args, model_args):
    # Fix for PyTorch 2.6's weights_only=True + DeepSpeed checkpoint deserialization
